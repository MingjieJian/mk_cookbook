...
\begin{aligned}
\kappa_\nu\rho & = N_l B_{lu} h\nu - N_uB_{ul} h\nu \\
&= N_l B_{lu} h\nu \left(1 - \frac{N_uB_{ul}}{N_l B_{lu}}\right) \\
&= N_l B_{lu} h\nu (1-e^{-h\nu/kT})
\end{aligned}
\tag{8.1}
\end{equation}
...
利用[第一章](OASP1.md)原子激发和电离的公式并将氢原子能级的可能态数代入：
...
\begin{equation}
\begin{aligned}
\alpha_\mathrm{ff}(\mathrm{H}) &= \frac{2}{3^{3/2}}\frac{h^2e^2R}{\pi m_e^3} \frac{1}{\nu^3} \int_0^\infty \left(\frac{2}{\pi}\right)^{1/2} \left(\frac{m}{kT}\right)^{3/2} v e^{-(mv^2/kT)} dv \\
&= \frac{2}{3^{3/2}}\frac{h^2e^2R}{\pi m_e^3} \frac{1}{\nu^3} \left(\frac{2m}{\pi kT}\right)^{1/2}
\end{aligned}
\tag{8.9}
\end{equation}
$$
